...
PYTHONPATH=src python3 -m trader paper
```
...
`paper` keeps a separate local ledger in `data/paper-state.json`. Run it once after each completed hourly candle with unchanged strategy settings. It records cash, BTC, estimated equity, and the next signal; a signal is filled at the *next candle's open* only after that candle has completed. If a run misses an hour, it stops rather than inventing a fill. This is a simulation with after-the-fact prices, not an executable order or a realistic live-fill guarantee. Keep the state file private and persistent; use `--state` to start a separate trial.

The live path is intentionally **not enabled or scheduled**. It requires a dedicated Coinbase Advanced portfolio holding only bot funds, an API key scoped to that portfolio with view/trade but **no transfer** permission, a private machine with persistent state, and explicit opt-in. Never put keys in this repository, the Pages site, or GitHub Actions. Install the optional SDK with `pip install -e '.[trade,dev]'`, then set `COINBASE_API_KEY` and `COINBASE_API_SECRET` as private environment variables. The live command requires `--portfolio-id`, `--execute`, and `LIVE_TRADING_ENABLED=YES`; do not run it until the strategy, venue, limits, credentials, and operational monitoring have been reviewed. It checks balances and product status, previews the order, caps estimated commission and price deviation, and records an attempt before submission. On an uncertain or submitted outcome it stops; run `PYTHONPATH=src python3 -m trader reconcile --portfolio-id YOUR_PORTFOLIO_ID` to look up the exact saved order with a view-only key. Reconciliation never places or retries an order, and it keeps the state unresolved if the order is absent, still open, or mismatched. The $9,800 portfolio-equity floor blocks new buys, not losses on an existing BTC position, and is **not** a guaranteed stop-loss. Use only one private host; the local lock does not coordinate multiple machines.

The design borrows Freqtrade's separate [dry-run/backtesting discipline](https://docs.freqtrade.io/en/stable/backtesting/) and LEAN's [live order reconciliation principle](https://www.quantconnect.com/docs/v2/writing-algorithms/live-trading/reconciliation), while retaining the [official Coinbase Advanced Python SDK](https://github.com/coinbase/coinbase-advanced-py). It does not import a full trading framework: Freqtrade does not officially support Coinbase spot, and a larger engine would not establish a profitable strategy or solve venue-specific fills here.
